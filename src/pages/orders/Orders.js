...
export default function Orders() {
  ...
  const onSearchChange = (e) => {
    ...
    const filterValue = delivery.filter((order) => {
      if (e.target.value == "") {
        return order;
      } else if (
  ...
  const getOnlineDeliveryOrders = () => {
    axios
      .get("/delivery-order/get-all-orders")
      .then((response) => {
  ...
  const getOnlineTakeAwayOrders = () => {
    axios
      .get("/takeaway-order/get-all-orders")
      .then((response) => {
  ...
  const completeOrders = (object) => {
    ...
    if (displayTrigger === 1) {
      axios
        .put(`/delivery-order/set-as-completed/${object._id}`)
        .then((response) => {
      ...
      axios
        .put(`/takeaway-order/set-as-completed/${object._id}`)
        .then((response) => {
          ...
          <>
            ...
            <IconButton>
              <DeleteIcon
                color="secondary"
                onClick={() =>
                  deleteOrders(params.row._id, params.row.is_completed)
                }
              />
            </IconButton>
            <IconButton>
  ...
  return (
    <div>
      ...
      <div className="rateandPdfRecord">
        <button
          className="generateRateRecord"
          onClick={exportPDFWithComponentDelivery}
        >
          {" "}
          Generate Report{" "}
        </button>
